...
public class QuadTreeNode {
     ...
     * when an item goes out of the QuadTree bounds. If there is no resize callback
     * set, the item is allowed to exist outside the QuadTree bounds, and resides
     * in the rootNode. A resize event is expensive.
     */
    protected QuadTreeResizeDelegate resize;
    
    /**
     * A reference to this nodes parent node. If null, then this node
     * is the rootNode.
     */
    ...
    /**
     * The maximum number of items that may exist in this node
     * before the node is split. There may exist more items than
     * the maximum, only if the items are on the boundaries of 2
     * child nodes.
     */
    ...
    protected void splitNode() {
        ...
        tlNode = new QuadTreeNode(this, new Rectangle(nLeft, nTop, nWidth, nHeight), maxItems);
        trNode = new QuadTreeNode(this, new Rectangle(midPoint.x, nTop, nWidth, nHeight), maxItems);
        blNode = new QuadTreeNode(this, new Rectangle(nLeft, midPoint.y, nWidth, nHeight), maxItems);
        brNode = new QuadTreeNode(this, new Rectangle(midPoint.x, midPoint.y, nWidth, nHeight), maxItems);
    ...
    public int getCompleteItemCount() {
        if(isSplit) {
            return tlNode.getCompleteItemCount() + trNode.getCompleteItemCount() + blNode.getCompleteItemCount() + brNode.getCompleteItemCount();
        }
    ...
    public void getItems(Point p, Class filters[], ArrayList<QuadTreeNodeItem> list) {
        if(this.getRect().contains(p)) {
            QuadTreeNodeItem[] items = Arrays.copyOf(itemList.toArray(), itemList.toArray().length, QuadTreeNodeItem[].class);
            for(QuadTreeNodeItem item : items) {
                if(filters != null) {
    ...
    public void getItems(Rectangle r, Class filters[], ArrayList<QuadTreeNodeItem> list) {
        if(this.getRect().intersects(r)) {
            QuadTreeNodeItem[] items = Arrays.copyOf(itemList.toArray(), itemList.toArray().length, QuadTreeNodeItem[].class);
            for(QuadTreeNodeItem item : items) {
                if(filters != null) {
    ...
    // <editor-fold defaultstate="expanded" desc="Destruction">
    ...
    protected void itemMove(QuadTreeNodeItem i) {
        if(itemList.contains(i)) {
            ...
            if(!pushItemDown(iIndex)) {
                if(parentNode != null) {
                    pushItemUp(iIndex);
                } else if (resize != null && !this.getRect().contains(i.getBoundingBox())) {
                    int newX = Math.min(RectUtility.getLeft(i.getBoundingBox()), RectUtility.getLeft(this.getRect()));
                    int newY = Math.min(RectUtility.getTop(i.getBoundingBox()), RectUtility.getTop(this.getRect()));
                    int newWidth = Math.max(RectUtility.getRight(i.getBoundingBox()), RectUtility.getRight(this.getRect())) - newX;
                    int newHeight = Math.max(RectUtility.getBottom(i.getBoundingBox()), RectUtility.getBottom(this.getRect())) - newY;

                    resize.resize(new Rectangle(   newX,
                                            newY,
                                            newWidth,
                                            newHeight));
                }
    ...
    public void renderTree(Rectangle rect, Point off, Color color, Graphics g) {
        ...
        if(rect.intersects(this.getRect())) {
            g.setColor(color);
            g.drawRect(this.getRect().x - off.x, this.getRect().y - off.y, this.getRect().width, this.getRect().height);
        }
    }
